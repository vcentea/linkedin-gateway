...
async def ensure_column(table: str, column: str, column_def: str):
    ...
    try:
        ...
        print(f"  ❌ Error ensuring column {table}.{column}: {e}")
        raise
...
async def ensure_schema():
    ...
    print("=" * 60)
    if fixed_count == 0:
        print("✅ Schema is correct - no changes needed")
    else:
        print(f"✅ Schema fixed - {fixed_count} item(s) added")
    print("=" * 60)
...
async def main():
    ...
    try:
        ...
        print(f"\n❌ Error during schema enforcement: {e}")
