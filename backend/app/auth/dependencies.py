...
async def get_requesting_user_from_api_key(
    ...
    # TODO: Consider updating last_used_at here? 
    # db_api_key.last_used_at = datetime.utcnow()
    # await db.commit() 

    logger.info(f"API key authentication successful for prefix '{prefix}', user ID: {db_api_key.user_id}")
    return db_api_key.user_id # Return the user ID associated with the valid key
...
async def validate_api_key_from_header_or_body(
    ...
    Returns:
        UUID: The user ID associated with the valid API key
