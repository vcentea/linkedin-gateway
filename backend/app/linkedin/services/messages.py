...
class LinkedInMessageService(LinkedInServiceBase):
    ...
    async def _get_my_profile_id(self) -> str:
        """
        Fetch the authenticated user's LinkedIn profile ID using centralized request flow.
        Uses GraphQL meMenu via self._make_request with standard service headers/cookies.
        """
        url = (
        ...
        logger.info("Fetching current user's profile ID via GraphQL meMenu (centralized)")
        # Add browser-like headers required by LinkedIn GraphQL
        graphql_headers = {
            **self.headers,
        ...
        data = await self._make_request(url, headers=graphql_headers)

        # Parse meMenu.*profile URN
        try:
            primary_items = data["data"]["data"]["feedDashGlobalNavs"]["primaryItems"]
            for item in primary_items:
                me_menu = item.get("meMenu")
                if me_menu:
                    user_urn = me_menu.get("*profile")
                    if user_urn and isinstance(user_urn, str):
                        match = re.search(r"urn:li:fsd_profile:([A-Za-z0-9_-]+)", user_urn)
                        if match:
                            profile_id = match.group(1)
                            logger.info(f"Retrieved my profile ID: {profile_id}")
                            return profile_id
        except Exception as e:
            logger.debug(f"Error parsing meMenu profile URN: {e}")

        raise ValueError("Could not extract profile ID from GraphQL meMenu response")
    ...
    async def prepare_send_message_request(
        ...
        message_text: str,
        my_profile_id: Optional[str] = None,
        ...
        Args:
            ...
            my_profile_id: Optional. The sender's LinkedIn profile ID (fetched if not provided).
        ...
        # Get my profile ID if not provided
        if not my_profile_id:
            my_profile_id = await self._get_my_profile_id()
