...
async def scrape_profile(
    ...
    # Validate API key
    user_id = await validate_api_key_from_header_or_body(
    ...
    if not request_body.server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[PROFILE_SCRAPE] User {user_id_str} not connected via WebSocket")
    ...
    try:
        ...
        profile_service = await get_linkedin_service(db, user_id, LinkedInProfileService)
...
async def scrape_profile_experiences(
    ...
    if not request_body.server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[EXPERIENCES] User {user_id_str} not connected via WebSocket")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User {user_id_str} WebSocket connection not found")
...
async def scrape_profile_recommendations(
    ...
    if not request_body.server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[RECOMMENDATIONS] User {user_id_str} not connected via WebSocket")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User {user_id_str} WebSocket connection not found")
