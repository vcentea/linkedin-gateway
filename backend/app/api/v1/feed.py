...
async def request_feed_posts(
    ...
    try:
        ...
        logger.info(f"[FEED] API Key validated for user ID: {requesting_user_id}")
    ...
    if not request_data.server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[FEED] User {user_id_str} not connected via WebSocket")
    ...
    try:
        # Get feed service to build URL and parse response
