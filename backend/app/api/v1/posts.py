...
async def request_posts_from_feed(
    ...
    **DEPRECATED**: Use `/api/v1/feed/posts` with `X-API-Key` header instead.
    ...
    try:
        ...
        logger.info(f"API Key validated for user ID: {requesting_user_id}")
    ...
    if request_data.server_call:
        ...
        try:
            ...
            feed_service = await get_linkedin_service(db, requesting_user_id, LinkedInFeedService)
    ...
    if user_id_str not in ws_handler.connection_manager.active_connections or not ws_handler.connection_manager.active_connections[user_id_str]:
        # Log details if check fails
        logger.warning(f"[WS Check Failed] User {user_id_str} not found in active connections or has empty set.")
        logger.warning(f"[WS Check Failed] Keys at time of failure: {active_conn_keys}")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="User not connected via WebSocket. Cannot request feed."
        )
...
async def get_profile_posts(
    ...
    if not request_body.server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[PROFILE_POSTS] User {user_id_str} not connected via WebSocket")
    ...
    logger.info(f"[PROFILE_POSTS][{mode}] Parameters - profile_id: {request_body.profile_id}, count: {request_body.count}")
    
    try:
        # Get service
        posts_service = await get_linkedin_service(db, requesting_user_id, LinkedInPostsService)
...
async def get_post_text(
    ...
    try:
        ...
        logger.info(f"[POST_TEXT] API Key validated for user ID: {requesting_user_id}")
    except HTTPException as auth_exc:
        raise auth_exc
    except Exception as e:
    ...
    if not request_body.server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[POST_TEXT] User {user_id_str} not connected via WebSocket")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User {user_id_str} WebSocket connection not found")
    ...
    try:
        # Get service to fetch and parse HTML
        posts_service = await get_linkedin_service(db, requesting_user_id, LinkedInPostsService)
