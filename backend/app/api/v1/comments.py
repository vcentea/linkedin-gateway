...
async def get_post_commenters(
    ...
    try:
        ...
        logger.info(f"[COMMENTERS] API Key validated for user ID: {requesting_user_id}")
    ...
    if not request_body.server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[COMMENTERS] User {user_id_str} not connected via WebSocket")
    ...
    try:
        # Get service to build URLs and parse responses
        comments_service = await get_linkedin_service(db, requesting_user_id, LinkedInCommentsService)
        ...
        while True:
            ...
            batch_commenters, next_pagination_token, batch_total, ugc_post_urn = comments_service._parse_commenters_response(raw_json_data, include_replies)
            ...
            logger.info(f"[COMMENTERS][{mode}] Waiting {delay:.2f}s before next request...")
...
async def post_comment_to_post(
    ...
    try:
        ...
        logger.info(f"[POST COMMENT] API Key validated for user ID: {requesting_user_id}")
    except HTTPException as auth_exc:
        raise auth_exc
    except Exception as e:
    ...
    if not server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[POST COMMENT] User {user_id_str} not connected via WebSocket")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User {user_id_str} WebSocket connection not found")
    ...
    try:
        # Get the comments service to prepare the request
        comments_service = await get_linkedin_service(db, requesting_user_id, LinkedInCommentsService)
...
async def reply_to_comment(
    ...
    try:
        ...
        logger.info(f"[REPLY TO COMMENT] API Key validated for user ID: {requesting_user_id}")
    except HTTPException as auth_exc:
        raise auth_exc
    except Exception as e:
    ...
    if not server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[REPLY TO COMMENT] User {user_id_str} not connected via WebSocket")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User {user_id_str} WebSocket connection not found")
