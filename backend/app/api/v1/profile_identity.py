...
async def scrape_profile_identity(
    ...
    if not request_body.server_call:
        ...
        if user_id_str not in ws_handler.connection_manager.active_connections or \
           not ws_handler.connection_manager.active_connections[user_id_str]:
            logger.warning(f"[IDENTITY] User {user_id_str} not connected via WebSocket")
    ...
    try:
        identity_service = await get_linkedin_service(db, user_id, LinkedInProfileIdentityService)
