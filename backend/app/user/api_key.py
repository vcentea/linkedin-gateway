...
from app.schemas.api_key import APIKeyResponse, APIKeyInfo, CSRFTokenUpdate, LinkedInCookiesUpdate
...
async def get_user_api_key(
    ...
    """
    Retrieve information about the current user's active API key.
    Returns 404 if no active key is found.
    """
    logger.info(f"Executing get_user_api_key for user ID: {current_user.id}")
    active_key = await api_key_crud.get_active_api_key_by_user(db=db, user_id=current_user.id)
    if not active_key:
    ...
    logger.info(f"Found active API key for user ID: {current_user.id}")
    # Ensure the response model includes the prefix now
    return APIKeyInfo.model_validate(active_key)
