#!/bin/bash
# LinkedIn Gateway - Open Core Teardown Script
# This script stops and optionally removes the open-core deployment
...
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}LinkedIn Gateway - Teardown${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
RUNNING_CONTAINERS=$(docker ps --filter "name=linkedin_gateway" --format "{{.Names}}" | wc -l)

if [ "$RUNNING_CONTAINERS" -eq 0 ]; then
    echo -e "  ${GREEN}✓ No LinkedIn Gateway containers running${NC}"
else
...
# Check volumes
VOLUMES=$(docker volume ls --filter "name=linkedingateway-saas" --format "{{.Name}}" | wc -l)
...
echo -e "  Redeploy:           cd $DEPLOYMENT_DIR/scripts && ./deploy-core.sh"
