...
export async function initWebSocket() {
  ...
  // --- Get User ID from per-server storage before connecting --- 
  let userId = null;
  try {
  ...
  const wsUrlWithUser = `${baseWssUrl}/${userId}`;
  ...
  // Store current server URL for future comparison
  currentServerUrl = baseWssUrl;
  
  logger.info(`[WEBSOCKET] Preparing to connect with userId: ${userId}`, 'websocket.service');
  logger.info(`[WEBSOCKET] Base WSS URL (DYNAMIC): ${baseWssUrl}`, 'websocket.service');
  logger.info(`[WEBSOCKET] Full WebSocket URL: ${wsUrlWithUser}`, 'websocket.service');
  
  // Create a new WebSocket connection
  try {
    logger.info(`[WEBSOCKET] Connecting to WebSocket server at: ${wsUrlWithUser}`, 'websocket.service');
