...
export async function initWebSocket() {
    ...
    try {
        const storage = await new Promise((resolve) => {
            chrome.storage.local.get([STORAGE_KEYS.USER_ID], resolve);
        });
        ...
        log(`[BG_WS] Error retrieving user ID for WebSocket: ${error.message}`);
    ...
    const wsUrlWithUser = `${WSS_URL}/${userId}`;
    
    log(`[BG_WS] Preparing to connect with userId: ${userId} to URL: ${wsUrlWithUser}`);
